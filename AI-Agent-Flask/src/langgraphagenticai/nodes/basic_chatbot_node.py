...
class StorageDecision(BaseModel):
    should_store: bool = Field(
        description="Whether to store the information - true or false"
    )
    message_to_store: str = Field(
        description="The clean, concise message to store (empty if not storing)"
    )
    reason: str = Field(description="Reason for the storage decision")
    is_duplicate: bool = Field(
        description="Whether this information is similar to existing stored information"
    )
...
class RestaurantRecommendationNode:
    ...
    def store_node(self, state: State) -> dict:
        ...
        if response.should_store and response.message_to_store:
            ...
            vectordb = Chroma.from_texts(
                texts=splits, embedding=embedding, persist_directory="./chroma_openai"
            )
    ...
    def evaluate_node(self, state: State) -> dict:
        ...
        class EvaluationResult(BaseModel):
            result: bool = Field(
                description="True if the answer is relevant and not an 'I don't know' response, False otherwise."
            )
    ...
    def search_node(self, state: State) -> dict:
        ...
        prompt_template = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    "You are a helpful assistant. Use the following chat history and search results to answer the user's latest question in a conversational, concise, and relevant way. If the results are not helpful, say you don't know.",
                ),
                (
                    "user",
        ...
        llm_response = self.llm.invoke(
            prompt_template.format(chat_history=chat_history_str, articles=articles_str)
        )
        # print(llm_response)
    ...
    def restaurant_node_sync(self, state: State) -> dict:
        """
        Processes the input state and generates a chatbot response.
        Now with MCP servers running, we can use the full functionality.
        """
        print("restaurant_node_sync called")
        try:
            # Run the async function in a new thread to avoid nested event loop
            ...
            def run_async():
                ...
                try:
                    return new_loop.run_until_complete(self.restaurant_node(state))
                finally:
                    new_loop.close()
            ...
            print(f"Error in restaurant_node_sync: {e}")
            # Fallback to simple response if MCP servers fail
            try:
                messages = state["messages"]
                if not messages:
                    return {"messages": "No messages to process"}
                ...
                # Create a simple prompt for sushi recommendations
                prompt = f"""You are a helpful assistant specializing in sushi restaurant recommendations in Munich. 
                The user asked: {user_input}
                
                Please provide helpful recommendations for sushi restaurants in Munich. 
